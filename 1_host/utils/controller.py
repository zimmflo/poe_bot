...
class VMHostPuppeteer:
  ...
  def connect(self):
    ...
    try:
      ...
      if self.debug:
        print(f"[Controller] established connection with {(self.host, self.port)}")
      self.connected = True
    except socket.error:
  ...
  def sendCommand(self, command: str, wait_till_recieved=True, recv_buffer_size=64):
    i_count = 0
    if wait_till_recieved is not False:
      command += "&wtr=1&"
    while self.sending:
      time.sleep(0.001)
      if self.debug:
        print("[Controller] debug blocking")
    self.sending = True
    ...
    while True:
      ...
      try:
        ...
        if self.debug:
          print(f"[Controller] debug finished {command} at {time.time()}")
        self.sending = False
  ...
  def getWindowLoc(self, window_name: str):
    ...
    img = self.sendCommandToRecieveBytes(command=action_msg)
    if self.debug:
      print("[Controller] got screenshot from")
    return img
  ...
  def getSortedByHSV(self, x1, y1, x2, y2, h1, s1, v1, h2, s2, v2):
    ...
    img = self.sendCommandToRecieveBytes(command=action_msg)
    if self.debug:
      print("[Controller] got sorted hsv points")
    return img
  ...
  def getClipboardText(self):
    ...
    action_msg = "action=getClipboardText&"
    img = self.sendCommandToRecieveBytes(command=action_msg)
    if self.debug:
      print("[Controller] got clipboard text")
    return img
  ...
  def checkProxy(self):
    action_msg = "action=checkProxy&"
    img = self.sendCommandToRecieveBytes(command=action_msg)
    if self.debug:
      print("[Controller] checked proxy")
    return img
...
class Keyboard:
  ...
  def pressAndRelease(self, button, delay=None, wait_till_executed=True, wait_till_recieved=False):
    ...
    while wait_till_executed and thread_finished[0] is not True:
      time.sleep(0.02)
...
class Mouse:
  current_pos_x = 0
  current_pos_y = 0
  pressed = {"right": False, "left": False, "middle": False}
  ...
  def press(self, x=-1, y=-1, button="left", force=False, wait_till_executed=True):
    if x != -1 and y != -1:
      self.internalAssignCursorPos(x, y)
    if self.pressed[button] is not True or force is True:
