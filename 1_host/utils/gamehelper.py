...
from .utils import (
  ...
  cropLine,
  getFourPoints,
  lineContainsCharacters,
  raiseLongSleepException,
)
...
class PoeBot:
  ...
  def __init__(
    ...
    unique_id,
    remote_ip,
    max_actions_per_second=random.randint(8, 10),
    debug=False,
    password=None,
    group_id="solo",
  ...
  def refreshInstanceData(
    self,
    refreshed_data=None,
    force=False,
    check_if_lag=0,
    reset_timer=False,
    raise_if_loading=False,
  ):
...
class Terrain:
  ...
  def getCurrentlyPassableArea(self, dilate_kernel_size=10):
    ...
    cv2.floodFill(
      currently_passable_dilated,
      None,
      (nearest_passable_player_point[1], nearest_passable_player_point[0]),
      floodval,
    )
  ...
  def pointToRunAround(
    self,
    point_to_run_around_x: int,
    point_to_run_around_y: int,
    distance_to_point=15,
    reversed=False,
    check_if_passable=False,
  ):
    poe_bot = self.poe_bot
    ...
    points_around = [
      ...
      [
        int(pos_x + distance_to_point * 0.7),
        int(pos_y - distance_to_point * 0.7),
      ],  # 45
  ...
  def clickTillNotTargetable(self, custom_break_condition=lambda *args, **kwargs: False):
    ...
    while True:
      ...
      for activator_search_i in range(20):
        activator = next(
          (e for e in self.poe_bot.game_data.entities.all_entities if e.id == self.id),
          None,
        )
        if activator:
...
class Player:
  ...
  def update(self, refreshed_data):
    ...
    self.grid_pos = PosXY(
      x=refreshed_data["gp"][0],
      y=self.poe_bot.game_data.terrain.terrain_image.shape[0] - refreshed_data["gp"][1],
    )
...
class Entities:
  ...
  def update(self, refreshed_data: dict):
    ...
    for raw_entity in refreshed_data["awake_entities"]:
      ...
      raw_entity["distance_to_player"] = dist(
        [raw_entity["gp"][0], raw_entity["gp"][1]],
