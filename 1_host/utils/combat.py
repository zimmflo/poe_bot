...
class LoopController:
  ...
  def start_new(self):
    poe_bot = self.poe_bot
    self.running = True
    print(f'[LoopController.start] thread starting')
    ...
    while self.running:
      ...
      if barrier_loop_running:
        if all(expected_change_history[-3:]):
          ...
          if changed_in_past == False:
            poe_bot.bot_controls.keyboard_releaseKey(self.build.barrier_invocation.skill_key , False)
            barrier_loop_running = False

      time.sleep(0.05)

    print(f'[LoopController.start] thread finished')
    if (self.build.barrier_invocation.skill_key  in poe_bot.bot_controls.keyboard.pressed):
      print(f'releasing button {self.build.barrier_invocation.skill_key }')
      poe_bot.bot_controls.keyboard_releaseKey(self.build.barrier_invocation.skill_key , False)
...
class BarrierInvocationInfernalist(Build):
  ...
  def usualRoutine(self, mover:Mover):
    ...
    if self.isInDemonForm() == True and mover.distance_to_target > 50:
      # distance to next step on screen
  ...
  def killUsual(self, entity, is_strong=False, max_kill_time_sec=10, *args, **kwargs):
    ...
    while True:
      ...
      if entity_to_kill.distance_to_player > keep_distance:
        ...
        point = self.poe_bot.game_data.terrain.pointToRunAround(entity_to_kill.grid_position.x, entity_to_kill.grid_position.y, kite_distance+random.randint(-1,1), check_if_passable=True, reversed=reversed_run)
        poe_bot.mover.move(grid_pos_x = point[0], grid_pos_y = point[1])
        if self.isInDemonForm() == True: self.dodge.use(wait_for_execution=False)
      else:
        print('[build.killUsual] kiting away')
        p0 = (entity_to_kill.grid_position.x, entity_to_kill.grid_position.y)
        p1 = (poe_bot.game_data.player.grid_pos.x, poe_bot.game_data.player.grid_pos.y)
  ...
  def __init__(self,poe_bot: PoeBot) -> None:
    ...
    for skill_index in range(len(self.poe_bot.game_data.skills.internal_names)):
      ...
      if skill_base_cast_time == None:
        ...
        self.minion_arconist_dd = SkillWithDelay(poe_bot=poe_bot, skill_index=skill_index, min_delay=random.uniform(3.0, 4.0), display_name="minion_arconist_dd", can_use_earlier=False)
      elif self.minion_reaver_enrage == None and skill_base_cast_time == 1000:
        print(f'[InfernalistMinion.__init__] found minion_reaver_enrage_index {skill_index}')
        self.minion_reaver_enrage = SkillWithDelay(poe_bot=poe_bot, skill_index=skill_index, min_delay=random.uniform(3.0, 4.0), display_name="minion_reaver_enrage", can_use_earlier=False)
      elif self.minion_sniper_gas_arrow == None and skill_base_cast_time == 1250:
        print(f'[InfernalistMinion.__init__] found minion_sniper_gas_arrow_index {skill_index}')
        self.minion_sniper_gas_arrow = SkillWithDelay(poe_bot=poe_bot, skill_index=skill_index, min_delay=random.uniform(2.5, 3.0), display_name="minion_sniper_gas_arrow", can_use_earlier=False)
    
    unearth_internal_name = "bone_cone"
    unearth_index = unearth_internal_name in self.poe_bot.game_data.skills.internal_names and self.poe_bot.game_data.skills.internal_names.index(unearth_internal_name)
    print(f'unearth_index {unearth_index}')
    self.unearth = None
    if unearth_index != False:
      self.unearth = SkillWithDelay(poe_bot=poe_bot, skill_index=unearth_index, min_delay=random.uniform(0.1, 0.2), display_name="unearth", can_use_earlier=True)    
    
    detonate_dead_internal_name = "detonate_dead"
    detonate_dead_index = detonate_dead_internal_name in self.poe_bot.game_data.skills.internal_names and self.poe_bot.game_data.skills.internal_names.index(detonate_dead_internal_name)
    print(f'detonate_dead_index {detonate_dead_index}')
    self.detonate_dead = None
    if detonate_dead_index != False:
      self.detonate_dead = SkillWithDelay(poe_bot=poe_bot, skill_index=detonate_dead_index, min_delay=random.uniform(0.5, 1.5), display_name="detonate_dead", can_use_earlier=False)
    ...
    self.offering = None
    if offerening_index != False:
      self.offering = SkillWithDelay(poe_bot=poe_bot, skill_index=offerening_index, min_delay=random.uniform(6.0, 8.0), display_name="offering", can_use_earlier=False)
    ...
    self.flammability = None
    if flammability_index != False:
      self.flammability = SkillWithDelay(poe_bot=poe_bot, skill_index=flammability_index, min_delay=random.uniform(2.0, 3.0), display_name="flammability", can_use_earlier=False)
  ...
  def killUsual(self, entity:Entity, is_strong = False, max_kill_time_sec = random.randint(200,300)/10, *args, **kwargs):
    ...
    while True:
      ...
      if skill_used is False and self.offering and self.offering.canUse():
        minions_around = list(filter(lambda e: e.is_hostile == False and e.distance_to_player < 35, poe_bot.game_data.entities.all_entities))
        if len(minions_around) != 0:
          if self.offering.use(updated_entity=minions_around[0], wait_for_execution=False) == True:
            skill_used = True
            
      if skill_used is False and self.flammability and self.flammability.canUse():
        if self.flammability.use(updated_entity=entity_to_kill, wait_for_execution=False) == True:
          skill_used = True
              
      if skill_used is False and self.minion_reaver_enrage and self.minion_reaver_enrage.canUse():
        if self.minion_reaver_enrage.use(updated_entity=entity_to_kill, wait_for_execution=False) == True:
          skill_used = True

      if skill_used is False and self.minion_sniper_gas_arrow and self.minion_sniper_gas_arrow.canUse():
        if self.minion_sniper_gas_arrow.use(updated_entity=entity_to_kill, wait_for_execution=False) == True:
          skill_used = True
