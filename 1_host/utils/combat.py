...
class CombatModule:
  ...
  def killTillCorpseOrDisappeared(
    self,
    entity: Entity,
    clear_around_radius=40,
    max_kill_time_sec=300,
    step_size=random.randint(30, 35),
  ):
    poe_bot = self.poe_bot
    ...
    if entity_to_kill.is_targetable is False or entity_to_kill.is_attackable is False:
      ...
      while True:
        ...
        if res is None:
          break
      entity_to_kill = next(
        (e for e in poe_bot.game_data.entities.all_entities if e.id == entity_to_kill_id),
        None,
      )
      if entity_to_kill is None:
        print("entity_to_kill is None corpse disappeared:")
        return True
      last_boss_pos_x, last_boss_pos_y = (
      ...
      while True:
        ...
        if entity_to_kill.is_targetable is False or entity_to_kill.is_attackable is False:
          ...
          killed_someone = self.clearLocationAroundPoint(
            {"X": entity_to_kill.grid_position.x, "Y": entity_to_kill.grid_position.y},
  ...
  def clearLocationAroundPoint(
    self,
    point_to_run_around,
    detection_radius=20,
    till_no_enemies_around=False,
    ignore_keys=[],
  ):
    """
    point_to_run_around
  ...
  def clearAreaAroundPoint(self, point, detection_radius=20, till_no_enemies_around=False, ignore_keys=[]):
    ...
    return self.clearLocationAroundPoint(
      point_to_run_around=point_dict,
      detection_radius=detection_radius,
      till_no_enemies_around=till_no_enemies_around,
...
class AutoFlasks:
  ...
  def useFlask(self, flask_index, flask_type="utility"):
    ...
    self.poe_bot.bot_controls.keyboard.pressAndRelease(
      f"DIK_{flask_index + 1}",
      delay=random.randint(15, 35) / 100,
      wait_till_executed=False,
    )
  ...
  def useUtilityFlasks(self):
    ...
    sorted_flasks = sorted(
      poe_bot.game_data.player.utility_flasks,
      key=lambda f: f.index,
      reverse=self.utility_flasks_use_order_reversed,
    )
  ...
  def useLifeFlask(self):
    ...
    if self.pathfinder is True:
      ...
      if need_to_use_flask is True:
        avaliable_life_flask = next(
          (f for f in poe_bot.game_data.player.life_flasks if f.can_use is not False),
          None,
        )
...
class AuraManager:
  ...
  def activateAurasIfNeeded(self):
    ...
    if not self.aura_skills:
      ...
      for skill_index in range(len(self.poe_bot.game_data.skills.internal_names)):
        ...
        if skill_name in aura_keys:
          ...
          if is_blessing:
            self.blessing_skill = BlessingSkill(
              poe_bot=self.poe_bot,
    ...
    if auras_to_activate:
      indexes_to_activate = list(
        map(
          lambda skill: self.poe_bot.game_data.skills.internal_names.index(skill),
          auras_to_activate,
        )
      )
    ...
    if updated_entity is not None or grid_pos_x != 0 or grid_pos_y != 0:  # if we need to move a mouse
      if updated_entity is not None:  # if its an entity
        screen_pos_x, screen_pos_y = (
          updated_entity.location_on_screen.x,
          updated_entity.location_on_screen.y,
        )
      else:
  ...
  def use(
    ...
    if (
      dist(
        ...
        [self.last_use_location[0], self.last_use_location[1]],
      )
      > self.area
    ):
      self.last_use_time = 0
...
class DetonateDead(Skill):
  def __init__(
    ...
    skill_index: int,
    skill_name="tipo chtobi potom uzat skill po ego internal name",
    display_name="DetonateDead",
    min_mana_to_use=0,
    desecrate: Skill = None,
  ) -> None:
  ...
  def use(
    ...
    if grid_pos_x != 0 or (updated_entity and updated_entity.life.health.current != 0):
      ...
      if not corpses:
        if self.desecrate:
          print("gonna cast desecrate cos there are no corpses around entity or pos")
  ...
  def staticDefence(self):
    ...
    if len(danger_zones) != 0:
      ...
      if self.chaos_immune is False and any(list(map(lambda s: "/LeagueArchnemesis/ToxicVolatile" in s, danger_zone_str))):
        ...
        caustic_orbs = list(
          filter(
            lambda e: "/LeagueArchnemesis/ToxicVolatile" in e.path,
            poe_bot.game_data.entities.all_entities,
          )
        )
        ...
        #
        pass
      elif self.chaos_immune is False and any(
        list(
          map(
            lambda s: "Metadata/Monsters/LeagueArchnemesis/LivingCrystal" in s,
            danger_zone_str,
          )
        )
      ):
        ...
        living_crystals = list(
          filter(
            lambda e: "Metadata/Monsters/LeagueArchnemesis/LivingCrystal" in e.path and e.distance_to_player < 20,
            danger_zones,
          )
        )
...
class GenericBuild2(Build):
  def __init__(self, poe_bot):
    ...
    main_attacking_skill = next(
      (s for s in self.poe_bot.game_data.skills.internal_names if s in GENERIC_BUILD_ATTACKING_SKILLS),
      None,
    )
    ...
    self.attacking_skill = SkillWithDelay(
      poe_bot=poe_bot,
      skill_index=skill_index,
      min_delay=random.randint(1, 5) / 100,
      display_name=main_attacking_skill,
      min_mana_to_use=0,
    )
  ...
  def usualRoutine(self, mover: Mover):
    ...
    nearby_enemies = list(
      filter(
        lambda e: e.isInRoi() and e.isInLineOfSight(),
        poe_bot.game_data.entities.attackable_entities,
      )
    )
...
class ButtonHolder:
  def __init__(
    ...
    custom_break_function=lambda: False,
  ):
    self.poe_bot = poe_bot
...
class BarrierInvocationInfernalist(Build):
  def __init__(self, poe_bot):
    ...
    demon_form = next(
      (s for s in poe_bot.game_data.skills.internal_names if s == "demon_transformation"),
      None,
    )
    if demon_form:
    ...
    barrier = next(
      (s for s in poe_bot.game_data.skills.internal_names if s == "barrier_invocation"),
      None,
    )
    if barrier:
  ...
  def usualRoutine(self, mover: Mover):
    ...
    pos_x_to_go, pos_y_to_go = (
      mover.nearest_passable_point[0],
      mover.nearest_passable_point[1],
    )
    if len(nearby_enemies) != 0:
...
class InfernalistZoomancer(Build):
  ...
  def __init__(self, poe_bot: PoeBot, can_kite=True) -> None:
    ...
    if detonate_dead_index is not False:
      self.detonate_dead = SkillWithDelay(
        ...
        min_delay=random.uniform(3.1, 4.5),
  ...
  def usualRoutine(self, mover: Mover = None):
    ...
    if mover is not None:
      ...
      nearby_enemies = list(
        filter(
          lambda e: e.distance_to_player < 50 and e.isInRoi(),
          poe_bot.game_data.entities.attackable_entities,
        )
      )
      ...
      if enemy_to_attack is not None:
        for _i in range(1):
          ...
          if self.detonate_dead and self.detonate_dead.canUse():
            corpses_around = poe_bot.game_data.entities.getCorpsesArountPoint(
              poe_bot.game_data.player.grid_pos.x,
              poe_bot.game_data.player.grid_pos.y,
              40,
            )
          ...
          if self.offering and self.offering.canUse():
            offering_spikes = list(
              filter(
                lambda e: "Metadata/Monsters/OfferingSpike/PainOfferingSpike" in e.path,
                poe_bot.game_data.entities.all_entities,
              )
            )
            if len(offering_spikes) == 0:
              alive_skeletons_nearby = list(
                filter(
                  lambda e: e.is_hostile is False and "Metadata/Monsters/Skeletons/PlayerSummoned/Skeleton",
                  poe_bot.game_data.entities.all_entities,
                )
              )
        ...
        enemies_on_way = list(
          filter(
            lambda e: e.distance_to_player < 15 and getAngle(p0, p1, (e.grid_position.x, e.grid_position.y), abs_180=True) < 45,
            really_close_enemies,
          )
        )
  ...
  def prepareToFight(self, entity: Entity):
    ...
    for i in range(random.randint(2, 3)):
      ...
      self.auto_flasks.useFlasks()
      updated_entity = next(
        (e for e in self.poe_bot.game_data.entities.all_entities if e.id == entity.id),
        None,
      )
