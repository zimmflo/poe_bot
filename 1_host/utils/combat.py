...
class TempestFlurryBuild(Build):
  def __init__(self, poe_bot):
    ...
    flurry_button = None
    print(f"looking for tempest flurry button on keyboard")
    for i in range(3,8): 
    ...
    def customStopMoving(self):
      q = [
        lambda:self.__class__.stopMoving(self),
        lambda:tempset_flurry_button_holder.forceStopPress()
      ]
      random.shuffle(q)
      while len(q) != 0:
        action = q.pop()
