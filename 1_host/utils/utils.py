...
import random
from math import cos, sin, radians, ceil, dist, degrees, atan2
import numpy as np
import cv2
...
def createLineIterator(P1, P2, swap_columns = True)->np.ndarray:
  ...
  # swap [[x,y]] to [[y,x]]
  if swap_columns: itbuffer[:, 0], itbuffer[:, 1] = itbuffer[:, 1], itbuffer[:, 0].copy()
...
def generateSnakeArray(display=False):
  ...
  if display: print(f'starting point is {starting_point}')
  ...
  while(len(snake_arr) < 60):
    ...
    if display: print(f'value_to_manipulate {value_to_manipulate}')
    ...
    for i in range(13):
      ...
      try:
        ...
        pass
      if display: print(current_point)
...
def cropLine(start, end, borders):
  ...
  # return (x,y)
  return points[-1]
def cropPath_depr(path, area_x = 25, area_y = 25, max_path_length = 100, current_pos_x = 999999999, current_pos_y = 999999999):
  '''
  supposed to crop the path in radius, and if the path is getting interrupted, it'll crop it till the break

  # TODO make it as ellipse rather than circle or square
  
  # TODO make it calculate the path from nearest point to us
  kinda get nearest point, get it's index, and start iterating since it
  nearest_point = [path[smth]]
  cropped_path = path[nearest_point:]
  

  '''
  if current_pos_x == 999999999:
    raise Exception("currentposx == 999999999 need to pass argument in it")
  cropped_path = []

  x_limit = area_x
  y_limit = area_y
  x_half = int(current_pos_x)
  y_half = int(current_pos_y)
  ...
  for point in path:
    if point[0] > y_range[0] and point[0] < y_range[1] and point[1] > x_range[0] and point[1] < x_range[1]:
      ...
      break
  cropped_path = cropped_path[-max_path_length:]
  return cropped_path
def cropPath(path, area_x = 25, area_y = 25, max_path_length = 100, current_pos_x = 999999999, current_pos_y = 999999999):
  '''
  supposed to crop the path in radius, and if the path is getting interrupted, it'll crop it till the break
  ...
  # print(f'#cropPath area_x {area_x} area_y {area_y} max_path_length = {max_path_length} current_pos_x {current_pos_x} current_pos_y {current_pos_y}')
  nearest_points = sorted(path, key=lambda p: dist( (p[0], p[1]), (current_pos_y, current_pos_x) ))
  nearest_point = nearest_points[0]
  nearest_point_index = np.where((path==nearest_point).all(1))[0][0]
  cropped_points = path[nearest_point_index:]
  nearest_dist = dist( (nearest_point[0], nearest_point[1]), (current_pos_y, current_pos_x) )
  # print(f"cropped path dist {nearest_dist} ")
  if nearest_dist > 50:
    return cropped_path
  ...
  for point in cropped_points:
    if point[0] > y_range[0] and point[0] < y_range[1] and point[1] > x_range[0] and point[1] < x_range[1]:
      ...
      break
  cropped_path = cropped_points[:max_path_length]
  return cropped_path
...
def generateSession(total_play_duration_h:int = 12, min_long_sleep_duration_h:int = 8, sessions_count:int = 4):
  session = []
