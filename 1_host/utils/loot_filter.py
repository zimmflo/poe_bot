...
class LootPicker:
  ...
  def collectLoot(self, *args):
    ...
    point_to_run_around = {
      "X": pickable_item.grid_position.x,
      "Y": pickable_item.grid_position.y,
    }
    ...
    if (
      dist(
        ...
        (pickable_item.grid_position.x, pickable_item.grid_position.y),
      )
      > 19
    ):
  ...
  def pickupDropV6(self, items_can_pick: List[PickableItemLabel]):
    ...
    if poe_bot.league == "Necropolis":
      ...
      for necropolis_tablet in necropolis_tablets:
        tablet_distance = dist(
          ...
          (
            nearest_pickable_item.grid_position.x,
            nearest_pickable_item.grid_position.y,
          ),
        )
    ...
    visible_labels_sorted = sorted(
      ...
      key=lambda label: dist(
        (
          (label["p_o_s"]["x1"] + label["p_o_s"]["x2"]) / 2,
          (label["p_o_s"]["y1"] + label["p_o_s"]["y2"]) / 2,
        ),
  ...
  def pickupDropV7(self, items_can_pick: List[PickableItemLabel]):
    ...
    def getLabelCenterPos(label_to_click):
      coords_to_click = (
        int((label_to_click["sz"][0] + label_to_click["sz"][1]) / 2),
        int((label_to_click["sz"][2] + label_to_click["sz"][3]) / 2),
      )
    ...
    for iter_num in range(4):
      ...
      targeted_item_can_pick = next(
        (i_raw for i_raw in visible_labels if i_raw["id"] in items_can_pick_at_the_same_time_ids and i_raw["it"] == 1),
        None,
      )
      ...
      if targeted_item_can_pick is None:
        last_targeted_item_id = 0
        updated_label = next(
          (i_raw for i_raw in visible_labels if i_raw["id"] == label_to_click["id"]),
          None,
        )
        if updated_label:
