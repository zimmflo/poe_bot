...
def checkIfCanTeleportToPartyMember(party_member_to_follow) -> bool:
  ...
  party_leader_img = game_img[
    party_member_to_follow["sz"][2] : party_member_to_follow["sz"][3],
    party_member_to_follow["sz"][0] : party_member_to_follow["sz"][1],
  ]
...
while True:
  ...
  if id_to_follow is None:
    ...
    portals_with_similar_area_name = next(
      (e for e in transitions_and_portals if e.render_name == member_to_follow_loc),
      None,
    )
...
plt.imshow(party_leader_img)
plt.show()
teleport_to_img = party_leader_img[30:43, 5:17]
plt.imshow(teleport_to_img)
plt.show()
sorted_img = sortByHSV(teleport_to_img, 76, 118, 109, 116, 213, 196)
