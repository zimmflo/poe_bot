...
class Mapper2(PoeBotComponent):
  ...
  def filterWaystonesCanRun(self, items: List[Item], sorted=False):
    waystones_can_run = list(
      filter(
        lambda i: i.map_tier and i.map_tier >= self.settings.min_map_tier and i.map_tier <= self.settings.max_map_tier,
        items,
      )
    )
    if sorted:
  ...
  def doPreparations(self):
    ...
    if need_to_modify_maps:
        ...
        if len(unidentified_items) != 0:
          while True:
            ...
            unidentified_items = list(
              filter(
                lambda m: m.map_tier and m.identified is False,
                poe_bot.ui.inventory.items,
              )
            )
            if len(unidentified_items) == 0:
              break
      ...
      if self.settings.waystone_upgrade_to_rare or self.settings.waystone_upgrade_to_rare_force:
        ...
        # aug logic
        maps_can_aug_regal = list(
          filter(
            lambda m: m.corrupted is False and m.identified is True and m.rarity == "Magic",
            maps_in_inventory,
          )
        )
        maps_can_aug = []
      ...
      if self.settings.anoint_maps:
        maps_can_be_anointed = list(
          filter(
            lambda m: m.corrupted is False and len(m.getDeliriumMods()) < 3,
            maps_in_inventory,
          )
        )
  ...
  def activateMap(self):
    ...
    if poe_bot.ui.map_device.place_map_window_opened is not True:
      ...
      nearest_map_objects = sorted(
        another_map_objects,
        key=lambda m: dist(map_obj.screen_zone.getCenter(), m.screen_zone.getCenter()),
      )
  ...
  def manageStashAndInventory(self, pick_consumables=False):
    ...
    if pick_consumables:
      ...
      maps_to_pick_count = random.randint(
        max([1, int(self.settings.keep_waystones_in_inventory * 0.6)]),
        self.settings.keep_waystones_in_inventory,
      )
      ...
      if all_waystones[0].source == "stash":
        ...
        list(
          map(
            lambda i: stash_tab_indexes.append(i.tab_index),
            self.getWaystonesCanUse(source="stash"),
          )
        )
  ...
  def exploreRoutine(self, *args, **kwargs):
    ...
    if current_area.activators_on_map:
      activators_on_map = list(
        filter(
          lambda e: e.is_targetable is not False and e.path in current_area.activators_on_map,
    ...
    if settings.force_kill_rares is not False:
      mob_to_kill = next(
        (e for e in poe_bot.game_data.entities.attackable_entities_rares if e.isOnPassableZone()),
        None,
      )
      if mob_to_kill:
        res = True
        while res is not None:
          ...
          mob_to_kill = next(
            (e for e in poe_bot.game_data.entities.attackable_entities_rares if e.id == mob_to_kill.id and e.isOnPassableZone()),
            None,
          )
    ...
    if settings.force_kill_blue is not False:
      mob_to_kill = next(
        (e for e in poe_bot.game_data.entities.attackable_entities_blue if e.isOnPassableZone()),
        None,
      )
      if mob_to_kill:
        poe_bot.combat_module.killUsualEntity(mob_to_kill)
    ...
    if settings.do_essences is not False:
      if len(poe_bot.game_data.entities.essence_monsters) != 0:
        ...
        return True
      essence = next(
        (e for e in poe_bot.game_data.entities.all_entities if e.is_targetable is True and ESSENCES_KEYWORD in e.path and e.isOnPassableZone()),
        None,
      )
      if essence:
    ...
    if settings.force_breaches is not False:
      breach_entity = next(
        (e for e in poe_bot.game_data.entities.all_entities if e.path == "Metadata/MiscellaneousObjects/Breach/BreachObject"),
        None,
      )
      if breach_entity:
  ...
  def onMapFinishedFunction(self):
    ...
    while time_now < map_finish_time + 1:
      ...
      if self.settings.complete_tower_maps is False:
        tower_activator_entity = next(
          (e for e in poe_bot.game_data.entities.all_entities if e.path == "Metadata/MiscellaneousObjects/Endgame/TowerCompletion"),
          None,
        )
    ...
    while True:
      ...
      if poe_bot.game_data.invites_panel_visible is not False:
        ...
        if random_click_iter > 15:
          ...
          point = poe_bot.game_data.terrain.pointToRunAround(
            ...
            distance_to_point=random.randint(15, 25),
        ...
        if len(nearby_portals) == 0:
          ...
          nearby_portals = list(
            filter(
              lambda e: e.distance_to_player < 50,
              poe_bot.game_data.entities.town_portals,
            )
          )
      ...
      poe_bot.combat_module.clearLocationAroundPoint(
        {
          "X": self.poe_bot.game_data.player.grid_pos.x,
          "Y": self.poe_bot.game_data.player.grid_pos.y,
        },
        detection_radius=50,
      )
      self.exploreRoutine()
...
def isItemHasPickableKey(item_label: PickableItemLabel):
  if item_label.icon_render in ARTS_TO_PICK:
    return True
  elif item_label.icon_render.startswith("Art/2DItems/Currency/") and "/Ruthless/" not in item_label.icon_render:
    return True
...
plt.imshow(
  poe_bot.game_data.terrain.currently_passable_area[
    arena_center[1] - 75 : arena_center[1] + 75,
    arena_center[0] - 75 : arena_center[0] + 75,
  ]
)
plt.show()
